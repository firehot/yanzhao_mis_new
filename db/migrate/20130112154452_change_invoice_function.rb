...
class ChangeInvoiceFunction < ActiveRecord::Migration
  def self.up
    ...
    if sf_group
      ...
      sf_cat.update_attributes(:system_function_group_id => sf_group.id,:default_action => 'common_invoices_path(:active_sub_tab => 5)') if sf_cat
    ...
    sf_cat.update_attributes(:default_action => "index_invoice_m_storages_path(:active_sub_tab => 11)") if sf_cat
  end
